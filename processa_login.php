...
require_once("connect.php");       // Conexão com Kami-Sama (Banco)
require_once("./classes/Usuario.php"); // Ficha do Guerreiro (Classe Usuario)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$email || !$senha) {
    ...
    echo json_encode(['success' => false, 'message' => 'Mestre Kame diz: Email e Senha são obrigatórios!']);
    exit();
  }
  ...
  if ($logado) {
    ...
    $agora = time();
    $servidor = 'SeuServidorCapsuleCorp.com'; // Quem está emitindo
    $paraQuem = 'UsuariosValidosApp';    // Para quem se destina

    $payload = [
      'iss' => $servidor,          // Emissor
